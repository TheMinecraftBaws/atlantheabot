...
public class Goodbye {
    public Goodbye(GuildMemberLeaveEvent event) {
        if (!event.getUser().isBot() && !event.getUser().isBot()) {
            ...
            if (event.getGuild().getTextChannelsByName(Variables.welcomeMessageChannel, true).size() > 0) {
                ...
                EmbedBuilder embedBuilder = new EmbedBuilder().setTitle("User Leave").setColor(Constants.RED)
                        .setDescription("Goodbye " + username + ". Thanks for visiting the Atlanthea Minecraft Discord.")
